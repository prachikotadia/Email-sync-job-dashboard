...
export default function Applications() {
  ...
  const [ statusFilter, setStatusFilter ] = useState( 'All Statuses' ); // Single-select status
  ...
  const performSearch = useCallback( async ( query, statusFilter = null, dateFromFilter = '', dateToFilter = '' ) => {
    if ( isGuest ) return;
    ...
    const hasActiveFilters = ( query && query.trim() ) || statusFilter || dateFromFilter || dateToFilter;
  ...
  const handleSearchChange = useCallback( ( value ) => {
    ...
    searchTimeoutRef.current = setTimeout( () => {
      performSearch( value, statusFilter !== 'All Statuses' ? statusFilter : null, dateFrom, dateTo );
    }, 300 );
  }, [ performSearch, statusFilter, dateFrom, dateTo ] );
  ...
  }, [ statusFilter, dateFrom, dateTo, searchParams, setSearchParams ] );

  // Effect to trigger search when status or dates change (separate from URL sync to avoid loops)
  useEffect( () => {
    // Skip on initial mount (let loadApplications handle it)
    if ( !applicationsLoaded ) return;

    // Debounce search when filters change
    const timeoutId = setTimeout( () => {
      const hasActiveFilters = ( statusFilter && statusFilter !== 'All Statuses' ) || dateFrom || dateTo || search.trim();
      if ( hasActiveFilters ) {
        performSearch( search, statusFilter !== 'All Statuses' ? statusFilter : null, dateFrom, dateTo );
      } else {
        // No filters - load all applications
  ...
  }, [ statusFilter, dateFrom, dateTo, search, applicationsLoaded, performSearch, loadApplications, companyFilter ] );
  ...
  const loadMoreApplications = useCallback( async () => {
    ...
    try {
      ...
      const result = await gmailService.unifiedSearch( {
        q: search?.trim() || null,
        status: statusFilter && statusFilter !== 'All Statuses' ? [ statusFilter ] : null,
        company: companyFilter || null,
  ...
  }, [ nextCursor, isGuest, loading, searchLoading, search, statusFilter, companyFilter, dateFrom, dateTo ] );
  ...
  const filtered = useMemo( () => {
    ...
    // Apply single-select status filter if not using search
    if ( statusFilter && statusFilter !== 'All Statuses' ) {
      list = list.filter( ( a ) => {
        // Use category if available, otherwise fallback to status
        const categoryValue = a.category || a.status || null;
    ...
    return list;
  }, [ applicationsAllTab, statusFilter, usingSearch ] );
              ...
              <div style={ { marginBottom: '1rem' } }>
                ...
                <select
                  ...
                  onChange={ ( e ) => {
                    ...
                    // Search will be triggered by useEffect watching statusFilter
                  } }
                  disabled={ isLoading }
                  style={ {
                    width: '100%',
                  ...
                  <div style={ { flex: 1 } }>
                    ...
                    <input
                      ...
                      onChange={ ( e ) => {
                        ...
                        setTimeout( () => {
                          performSearch( search, statusFilter !== 'All Statuses' ? statusFilter : null, e.target.value, dateTo );
                        }, 100 );
                      } }
                      className="filter-search-input"
                      disabled={ isLoading }
                  ...
                  <div style={ { flex: 1 } }>
                    ...
                    <input
                      ...
                      onChange={ ( e ) => {
                        ...
                        setTimeout( () => {
                          performSearch( search, statusFilter !== 'All Statuses' ? statusFilter : null, dateFrom, e.target.value );
                        }, 100 );
                      } }
                      className="filter-search-input"
                      disabled={ isLoading }
